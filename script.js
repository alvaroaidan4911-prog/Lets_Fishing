...
const jetskiSpawnPos=new THREE.Vector3(10,-0.3,8);
...
lava.position.set(-600,-502.5,-500); scene.add(lava);
...
function buildShop(px,pz,label){
  ...
  [new THREE.Mesh(new THREE.BoxGeometry(10,7,0.4),wm),
   ...
   new THREE.Mesh(new THREE.BoxGeometry(0.4,7,6),wm)].forEach((m,i)=>{
    if(i===0)m.position.set(0,1.5,-3);
    else if(i===1)m.position.set(-4.8,1.5,0);
    else m.position.set(4.8,1.5,0);
    g.add(m);
  });
  ...
  sg.position.set(0,5.9,2.9); g.add(sg);
...
jetski.position.copy(jetskiSpawnPos); scene.add(jetski);
...
function movePlayer(dt){
  ...
  // Vertical
  if(isSwimming){
    player.position.y=THREE.MathUtils.lerp(player.position.y,-0.55,0.1);
  } else {
...
function mountJetski(){
  if(!jetskiOwned){showMessage("🛥️ Buy a Jetski from the Jetski Shop!");return;}
  onJetski=true;isSwimming=false;uwDiv.style.display="none";
  player.position.copy(jetski.position);player.position.y=0.5;
  armL.rotation.x=-0.3;armR.rotation.x=-0.3;
  torso.rotation.x=0;legL.rotation.x=0;legR.rotation.x=0;
  document.getElementById("jetskiUI").style.display="block";
  showMessage("🛥️ Riding! [WASD/Joystick] drive · [E] dismount");
}
...
function updateJetski(){
  ...
  jetski.position.y=-0.3+Math.sin(Date.now()*0.002)*0.08;
  jetski.rotation.x=jetskiSpeed*0.18; jetski.rotation.z=-mX*0.07;
  player.position.copy(jetski.position);player.position.y=jetski.position.y+0.9;
  player.rotation.y=jetski.rotation.y; torso.position.y=3;
...
function updateNPCInteraction(){
  ...
  // Sell fish
  if(player.position.distanceTo(fishPos)<8){
  ...
  // Rod shop
  if(player.position.distanceTo(rodPos)<8){
  ...
  // Bait / jetski shop
  if(player.position.distanceTo(baitPos)<8||player.position.distanceTo(jsPos)<8){
  ...
  if(nearJetski&&!onJetski&&jetskiOwned){
    ...
    mountBtn.textContent="🛥️ Dismount [E]";mountBtn.style.display="block";
  } else{mountBtn.textContent="🛥️ Mount [E]";mountBtn.style.display="none";}

  // Hint
  const hint=document.getElementById("interactHint");
  if(player.position.distanceTo(fishPos)<8)hint.textContent="🐟 Near Sell Shop — press Sell";
  else if(player.position.distanceTo(rodPos)<8)hint.textContent="🎣 Rod Shop nearby [E]";
  else if(player.position.distanceTo(baitPos)<8)hint.textContent="🪱 Bait Shop / Jetski Shop";
  else if(nearJetski&&!onJetski)hint.textContent="🛥️ Press [E] or button to mount";
...
document.getElementById("openJetskiShopBtn").addEventListener("click",()=>{
  ...
  if(player.position.distanceTo(baitPos)<8){toggleInventory();if(inventoryOpen)switchTab("bait");}
...
window.addEventListener("keydown",e=>{
  ...
  if(k==="e"){if(onJetski){dismountJetski();return;}if(nearJetski){mountJetski();return;}}
...
document.getElementById("slot1").addEventListener("click",()=>{
  if(inventory.equipped==="FishingRod"){unequipRod();document.getElementById("slot1").classList.remove("active");}
  else{equipRod("FishingRod");document.getElementById("slot1").classList.add("active");}
});
function unequipRod(){
