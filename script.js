...
const jetskiSpawnPos=new THREE.Vector3(10,0.7,8);
...
lava.position.set(-600,-501.5,-500); scene.add(lava);
...
function movePlayer(dt){
  ...
  // Vertical
  if(isSwimming){
    player.position.y=THREE.MathUtils.lerp(player.position.y,-1.8,0.1); // di bawah permukaan air
  } else {
    player.position.y=THREE.MathUtils.lerp(player.position.y,0,0.15); // y=0 = on land/island
  }
...
function updateJetski(){
  ...
  jetski.position.y=0.7+Math.sin(Date.now()*0.002)*0.08;
